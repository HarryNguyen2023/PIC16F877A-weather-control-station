...
# 2 "<built-in>" 2
# 1 "Master_ModbusRTU.c" 2
# 1 "./Master_ModbusRTU.h" 1
...
# 4 "./Master_ModbusRTU.h" 2
# 15 "./Master_ModbusRTU.h"
uint8_t ModbusFrame[4 + 5];
__bit send = 0;
...
void Master_SendCommand(uint8_t sla_add, uint8_t function){
    if(send == 0){
        ...
        ModbusFrame[4] = 0x63;
        ModbusFrame[5] = 0x64;
        uint16_t CRC = CRCcheck(ModbusFrame,4 + 2);
...
uint16_t CRCcheck(uint8_t buf[], uint8_t len){
    ...
    for (uint16_t pos = 0; pos < len; pos++)
    {
        ...
        for (uint16_t index = 8; index != 0; index--)
        {
            if ((crc & 0x0001) != 0){
                crc >>= 1;
                crc ^= 0xA001;
